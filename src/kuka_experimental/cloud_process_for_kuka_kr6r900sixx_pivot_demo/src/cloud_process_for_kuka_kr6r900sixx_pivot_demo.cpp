...
int main(int argc, char **argv)
{
  ...
  std::string modelName2 = (std::string)"cylinder2";
  gazebo_msgs::SetModelState setModelState;
  gazebo_msgs::ModelState modelstate_cylinder2;
  
  modelstate_cylinder2.model_name      = modelName2;
  modelstate_cylinder2.reference_frame = "base_link";
  
  geometry_msgs::Pose   cylinder2_pose_in_base_link;
  geometry_msgs::Pose   cylinder2_pose_in_tool0_link;
  ...
  Eigen::Vector3d posi_cylinder2_in_tool0;
  posi_cylinder2_in_tool0 << 0.23, 0, 0;
  Eigen::Vector3d posi_cylinder2_in_base_link;
  posi_cylinder2_in_base_link << 0, 0, 0;
...
*/
  ...
  while (ros::ok()) {
    ...
    */
    /*
    //Service to set cylinder2 pose
    cylinder2_pose_in_base_link.position.x    = pp.x;
    cylinder2_pose_in_base_link.position.y    = pp.y;
    cylinder2_pose_in_base_link.position.z    = pp.z;
    cylinder2_pose_in_base_link.orientation.x = qq.x;  
    cylinder2_pose_in_base_link.orientation.y = qq.y; 
    cylinder2_pose_in_base_link.orientation.z = qq.z; 
    cylinder2_pose_in_base_link.orientation.w = qq.w; 
    */
    
    //Transform cylinder2_pose_in_base_link to cylinder2_pose_in_tool0_link
     //First get the tf frome base_link to tool0
    ...
    posi_cylinder2_in_base_link = full_rotation_to_tete_beche*posi_cylinder2_in_tool0;
    //posi_cylinder2_in_base_link = eigein_quat_from_tool0_to_base_link*posi_cylinder2_in_tool0;


	
    //Service to set cylinder2 pose
    cylinder2_pose_in_base_link.position.x    = pp.x+posi_cylinder2_in_base_link[0];
    cylinder2_pose_in_base_link.position.y    = pp.y+posi_cylinder2_in_base_link[1];
    cylinder2_pose_in_base_link.position.z    = pp.z+posi_cylinder2_in_base_link[2];
    cylinder2_pose_in_base_link.orientation.x = qq_tete_beche.x;  
    cylinder2_pose_in_base_link.orientation.y = qq_tete_beche.y; 
    cylinder2_pose_in_base_link.orientation.z = qq_tete_beche.z; 
    cylinder2_pose_in_base_link.orientation.w = qq_tete_beche.w; 
	
	
	
    modelstate_cylinder2.pose   = cylinder2_pose_in_base_link;
    setModelState.request.model_state = modelstate_cylinder2;
    ros::ServiceClient client2  = nh.serviceClient<gazebo_msgs::SetModelState>("/gazebo/set_model_state");
    client2.call(setModelState);
    ...
    //Broadcast the tf between the child_link=cylinder2_link and the parent link=base_link
    static tf::TransformBroadcaster broadcaster2;
    broadcaster2.sendTransform(tf::StampedTransform(tf::Transform(tf::Quaternion(qq_tete_beche.x, qq_tete_beche.y, qq_tete_beche.z, qq_tete_beche.w), tf::Vector3(pp.x+posi_cylinder2_in_base_link[0], pp.y+posi_cylinder2_in_base_link[1], pp.z+posi_cylinder2_in_base_link[2])), ros::Time::now(), "base_link", "cylinder2_link"));
